...
static bool is_xcfi_ret2ns_forced_fault_pattern(const struct exception_info_t *ctx)
{
#if defined(FAULT_STATUS_PRESENT)
    ...
    return (ctx->VECTACTIVE == EXCEPTION_TYPE_HARDFAULT)
           && (ctx->EXC_FRAME_COPY[6] == 0U)
           && (((ctx->HFSR | ctx->HFSR_NS) & SCB_HFSR_FORCED_Msk) != 0U);
#else
    (void)ctx;
                                ...
                                const struct exception_info_t *ctx)
{
    ...
    if (!is_return_secure_stack(ctx->EXC_RETURN)) {
        SPMLOG_DBGMSGVAL("    CFSR_NS: ", ctx->CFSR_NS);
        SPMLOG_DBGMSGVAL("    UFSR_NS: ",
                        (ctx->CFSR_NS & SCB_CFSR_USGFAULTSR_Msk) >> SCB_CFSR_USGFAULTSR_Pos);
        SPMLOG_DBGMSGVAL("    HFSR_NS: ", ctx->HFSR_NS);
    }
#endif

    if (is_xcfi_ret2ns_forced_fault_pattern(ctx)) {
        if (((ctx->CFSR | ctx->CFSR_NS) & SCB_CFSR_INVSTATE_Msk) != 0U) {
            SPMLOG_ERRMSG("[XCFI] PAC/AUT return authentication failure confirmed by UFSR.INVSTATE during ret2ns secure return.\r\n");
        } else {
            SPMLOG_ERRMSG("[XCFI] ret2ns secure-return fault reached, but PAC/AUT is not architecturally confirmed.\r\n");
            SPMLOG_ERRMSG("[XCFI] UFSR.INVSTATE is clear; inspect CFSR/CFSR_NS and ret2ns_svc_progress.\r\n");
        }
    }
                            ...
                            uint32_t *callee_saved)
{
    ...
    ctx->CFSR_NS = SCB_NS->CFSR;
    ctx->HFSR_NS = SCB_NS->HFSR;
    ctx->BFAR_NS = SCB_NS->BFAR;
    ctx->BFARVALID_NS = ctx->CFSR_NS & SCB_CFSR_BFARVALID_Msk;
    ctx->MMFAR_NS = SCB_NS->MMFAR;
    ctx->MMARVALID_NS = ctx->CFSR_NS & SCB_CFSR_MMARVALID_Msk;
    SCB_NS->CFSR = ctx->CFSR_NS; /* Clear bits. CFSR is write-one-to-clear. */
    SCB_NS->HFSR = ctx->HFSR_NS; /* Clear bits. HFSR is write-one-to-clear. */
#endif
#endif
