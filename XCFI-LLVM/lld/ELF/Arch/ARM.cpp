...
ArmCmseSGSection::ArmCmseSGSection(Ctx &ctx)
    : SyntheticSection(ctx, ".gnu.sgstubs", SHT_PROGBITS,
                       ...
                       /*addralign=*/32) {
  if (ctx.arg.enableXCFILLD) entsize = 36;
  else entsize = ACLESESYM_SIZE;
...
void ArmCmseSGSection::addSGVeneer(Symbol *acleSeSym, Symbol *sym) {
  // if (ctx.arg.enableXCFILLD) {
    llvm::errs() << "[DEBUG] XCFI Option Enabled\n";
    llvm::errs() << "  - armCMSESupport: " << ctx.arg.armCMSESupport << "\n";
    llvm::errs() << "  - cmseImplib: " << ctx.arg.cmseImplib << "\n";
    llvm::errs() << "  - Sym: " << sym->getName() << " Val: " << cast<Defined>(*sym).value << " File: " << sym->file << "\n";
    llvm::errs() << "  - ACLE: " << acleSeSym->getName() << " Val: " << cast<Defined>(*acleSeSym).value << " File: " << acleSeSym->file << "\n";
  // }
...
static uint16_t readSidFromAcleSym(Symbol *sym) {
  auto *d = dyn_cast<Defined>(sym);
  if (!d)
    fatal("acleSeSym is not a Defined symbol");

  auto *isec = dyn_cast<InputSectionBase>(d->section);
  if (!isec)
    fatal("acleSeSym has no input section");

  llvm::ArrayRef<uint8_t> data = isec->content();

  uint64_t off = d->value;
  if (off < 2) fatal("function offset < 2");
  return read16le(data.data() + off - 2);
}
...
void ArmCmseSGSection::writeTo(uint8_t *buf) {
  for (std::unique_ptr<ArmCmseSGVeneer> &s : sgVeneers) {
    if (!ctx.arg.enableXCFILLD) {
        ...
        Symbol *XCFISym = ctx.symtab->find("XCFI_fault");
        if (!XCFISym)
            Err(ctx) << "XCFI_fault not found";
        
        write16(ctx, p + 0, 0xe97f); // SG
        write16(ctx, p + 2, 0xe97f);
        write16(ctx, p + 4, 0xf84d); // str.w r12, [sp, #-4]
        write16(ctx, p + 6, 0xcc04);
        write16(ctx, p + 8, 0xf8df); // ldr.w r12, [pc, #0x14]
        write16(ctx, p + 10, 0xc014);
        write16(ctx, p + 12, 0xfa1f); // uxth.w r12, r12
        write16(ctx, p +14, 0xfc8c);
        write16(ctx, p +16, 0xebbc); // subs.w r12, r12, r10
        write16(ctx, p +18, 0x0c0a);

        const intptr_t branchOff = 20;
        write16(ctx, p +20, 0xf041); // bne.w XCFI_fault
        write16(ctx, p +22, 0x80e7);
        uint64_t instrAddr = getVA() + s->offset + branchOff;
        uint64_t pcForReloc = instrAddr + 4;
        int64_t relValue = (int64_t)XCFISym->getVA(ctx) - (int64_t)pcForReloc;
        ctx.target->relocateNoSym(p+branchOff, R_ARM_THM_JUMP19, relValue);

        write16(ctx, p +24, 0xf85d); // ldr.w r12, [sp, #-4]
        write16(ctx, p +26, 0xcc04);

        const intptr_t blOff = 28;
        write16(ctx, p + blOff    , 0xf000); // B.W S
        write16(ctx, p + blOff + 2, 0xb000);
        instrAddr = getVA() + s ->offset + blOff;
        pcForReloc = instrAddr + 4;
        relValue = (int64_t)s->acleSeSym->getVA(ctx) - (int64_t)pcForReloc;
        ctx.target->relocateNoSym(p + blOff, R_ARM_THM_JUMP24, relValue);

        uint16_t sid = readSidFromAcleSym(s->acleSeSym);
        uint16_t uid = hash_value(s->sym->getName());
        if(uid == 0xf3af || uid == 0xe97f) uid = hash_value(uid); // 0xf3af is bti, 0xe97f is sg
        uint32_t cid = (uid << 16) | sid;
        // evaluation only
        cid = 0xdeadbeef;
        // evaluation
        write32(ctx, p+32, cid);
...
void ArmCmseSGSection::finalizeContents() {
  ...
  for (auto [i, s] : enumerate(sgVeneers)) {
    if (!ctx.arg.enableXCFILLD) {
        ...
        s->offset = i*entsize; 
        Defined(ctx, file, StringRef(), s->sym->binding, s->sym->stOther,
            s->sym->type, s->offset | 1, entsize, this)
        .overwrite(*s->sym);
    }
  }
}
