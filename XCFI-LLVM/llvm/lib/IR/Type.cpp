...
std::string getTypeEncoding(Type *Ty) {
    if (Ty->isHalfTy()) return "half";
    if (Ty->isBFloatTy()) return "f16";
    if (Ty->isFloatTy()) return "f32";
    if (Ty->isDoubleTy()) return "d";
    if (Ty->isVoidTy()) return "v";
    ...
    if (Ty->isFunctionTy()) return "func";
    if (Ty->isPointerTy()) return "ptr";
    if (Ty->isStructTy()) return "struct";
    if (Ty->isArrayTy()) return "arr";
    return "u";
}

uint16_t Type::getSID() const {
    assert(getTypeID() == FunctionTyID && "getFID should called on FunctionType");

    const FunctionType *FTy = cast<FunctionType>(this);
    std::string sig = "";

    Type *RetTy = FTy->getReturnType();
    sig += getTypeEncoding(RetTy);

    unsigned numParams = FTy->getNumParams();
    sig += "_n" + std::to_string(numParams);

    for (unsigned i=0; i<numParams; i++) {
        Type *ArgTy = FTy->getParamType(i);
        sig += "_" + getTypeEncoding(ArgTy);
    }

    auto hash = hash_value(sig);
    uint16_t fid = static_cast<uint16_t>((hash >> 16) ^ (hash & 0xffff));
    return fid;
}
//
