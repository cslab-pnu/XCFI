...
// xcfi
  if (XCFIEnabled()) {
      Align A = MF->getAlignment();
      if (A < Align(4)) A = Align(4);
      OutStreamer->emitValueToAlignment(A, 0x00, 1, 0);

      MCSymbol *DataMS = OutContext.getOrCreateSymbol("__fid_" + CurrentFnSym->getName().str());
      OutStreamer->emitLabel(DataMS);

      uint16_t sid = MF->getFunction().getFunctionType()->getSID();
      uint16_t uid = hash_value(MF->getFunction().getName());
      if(uid == 0xf3af || uid == 0xe97f) uid = hash_value(uid); // 0xf3af is bti, 0xe97f is sg
      uint32_t cid = (uid << 16) | sid;
      // evaluation only
      cid = 0xdeadbeef;
      // evaluation
      OutStreamer->emitIntValue(cid, 4);
  }
//
