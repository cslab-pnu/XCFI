  ...
  if (offset >= 0 && offset <= 4092) {
    ...
    if (Spill) {
      ...
      NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::INLINEASM))
            .addExternalSymbol(MF.createExternalSymbolName("str r"+std::to_string(ScratchReg.id()-73)+", [sp, #-4]\n"))
            .addImm(1)
            .addExternalSymbol(""));
    }
    ...
    if (Spill) {
      ...
      //                  .add(predOps(Pred, PredReg))
      //                  .addReg(ScratchReg));
      // NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::t2LDRi12))
      //                   .addReg(ScratchReg)
      //                   .addReg(ARM::SP)
      //                   .addImm(-4)
      //                   .add(predOps(Pred, PredReg)));
      NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::INLINEASM))
            .addExternalSymbol(MF.createExternalSymbolName("ldr r"+std::to_string(ScratchReg.id()-73)+", [sp, #-4]\n"))
            .addImm(1)
            .addExternalSymbol(""));
    }
  }
...
ARMSilhouetteShadowStack::popFromShadowStack(MachineInstr & MI,
  ...
  if(MI.getOpcode() == ARM::t2LDR_POST && MI.getOperand(0).getReg() == ARM::LR && MI.getOperand(1).getReg() == ARM::SP){
    // MI.dump();
    int tmp = MI.getOperand(3).getImm();
    tAddImm = tmp/4;
  }
  ...
  if (offset >= 0 && offset <= 4092) {
    // Single-instruction shortcut
    // If PC, load to LR first then use BX_RET
    Register TargetReg = PCLR.getReg();
    bool NeedReturn = (TargetReg == ARM::PC);
    if (NeedReturn) {
      TargetReg = ARM::LR;
      // Mark LR as restored
    ...
    NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::t2LDRi12), TargetReg)
                     .addReg(ARM::SP)
                     .addImm(offset)
                     .add(predOps(Pred, PredReg))
                     .setMIFlag(MachineInstr::ShadowStack));
    ...
    // If PC and we didn't already change to LR (due to spill), load to LR first then use BX_RET
    Register TargetReg = PCLR.getReg();
    bool NeedReturnForLDRs = (TargetReg == ARM::PC && !NeedReturn);
    if (NeedReturnForLDRs) {
      TargetReg = ARM::LR;
      // Mark LR as restored
      MachineFrameInfo &MFI = MF.getFrameInfo();
      if (MFI.isCalleeSavedInfoValid()) {
    ...
    NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::t2LDRs), TargetReg)
                     .addReg(ARM::SP)
                     .addReg(ScratchReg)
                     .addImm(0)
                     .add(predOps(Pred, PredReg))
    ...
    // Insert a return if needed (either from spill case or non-spill PC case)
    if (NeedReturn || NeedReturnForLDRs) {
      NewMIs.push_back(BuildMI(MF, DL, TII->get(ARM::tBX_RET))
                       .add(predOps(Pred, PredReg)));
    }
  }
...
ARMSilhouetteShadowStack::runOnMachineFunction(MachineFunction & MF) {
  ...
  if (MF.getFunction().getName() == "z_arm_fault" || MF.getFunction().getName() == "relocate_vector_table" || MF.getFunction().getName() == "main" ||
  MF.getFunction().getName() == "z_arm_floating_point_init" || MF.getFunction().getName() == "z_arm_interrupt_init" || MF.getFunction().getName() == "gpio_cmsdk_port_3_config_func" ||
  MF.getFunction().getName() == "z_cstart" || MF.getFunction().getName() == "z_arm_fault_init" || MF.getFunction().getName() == "gpio_cmsdk_port_2_config_func" ||
  MF.getFunction().getName() == "z_arm_cpu_idle_init" || MF.getFunction().getName() == "z_arm_mpu_init" || MF.getFunction().getName() == "z_arm_configure_static_mpu_regions" || 
  MF.getFunction().getName() == "arm_core_mpu_configure_static_mpu_regions" || MF.getFunction().getName() == "arm_core_mpu_mark_areas_for_dynamic_regions" ||
  MF.getFunction().getName() == "z_dummy_thread_init" || MF.getFunction().getName() == "z_device_state_init" || MF.getFunction().getName() == "k_object_init" ||
  MF.getFunction().getName() == "z_sys_init_run_level" || MF.getFunction().getName() == "statics_init" || MF.getFunction().getName() == "do_device_init" ||
  MF.getFunction().getName() == "gpio_cmsdk_ahb_init" || MF.getFunction().getName() == "gpio_cmsdk_port_0_config_func" || MF.getFunction().getName() == "arch_data_copy" ||
  MF.getFunction().getName() == "arm_irq_priority_set" || MF.getFunction().getName() == "arm_irq_enable" || MF.getFunction().getName() == "init_mem_domain_module" ||
  MF.getFunction().getName() == "arm_irq_disable" || MF.getFunction().getName() == "gpio_cmsdk_port_1_config_func" || MF.getFunction().getName() == "arch_mem_domain_max_partitions_get" ||
  MF.getFunction().getName() == "k_mem_domain_add_partition" || MF.getFunction().getName() == "app_shmem_bss_zero" || MF.getFunction().getName() == "gpio_mmio32_init" || MF.getFunction().getName() == "uart_cmsdk_apb_init" ||
  MF.getFunction().getName() == "pinctrl_lookup_state" || MF.getFunction().getName() == "uart_cmsdk_apb_irq_config_func_0" || MF.getFunction().getName() == "uart_cmsdk_apb_irq_config_func_1" ||MF.getFunction().getName() == "uart_cmsdk_apb_irq_config_func_2" ||
  MF.getFunction().getName() == "uart_cmsdk_apb_irq_config_func_3" || MF.getFunction().getName() == "uart_cmsdk_apb_irq_config_func_4" || MF.getFunction().getName() == "pinctrl_configure_pins" ||
  MF.getFunction().getName() == "cmsdk_ahb_gpio_config" || MF.getFunction().getName() == "uart_console_init" || MF.getFunction().getName() == "__stdout_hook_install" || MF.getFunction().getName() == "__printk_hook_install" ||
  MF.getFunction().getName() == "sys_clock_driver_init" || MF.getFunction().getName() == "prepare_multithreading" || MF.getFunction().getName() == "z_sched_init" || MF.getFunction().getName() == "z_setup_new_thread" ||
  MF.getFunction().getName() == "z_impl_k_object_access_grant" || MF.getFunction().getName() == "arch_tls_stack_setup" || MF.getFunction().getName() == "mpu_configure_region") {
