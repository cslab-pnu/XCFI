...
ARMSilhouetteLabelCFI::runOnMachineFunction(MachineFunction & MF) {
  ...
  if (MF.getFunction().getName() == "z_arm_fault") {
  ...
  if ((!F.hasInternalLinkage() && !F.hasPrivateLinkage()) ||
      F.hasAddressTaken() || F.getName() == "setmask" || F.getName() == "getBits1" || F.getName() == "getBits2" || F.getName() == "add_huff_table" ||
      F.getName() == "read_byte" || F.getName() == "emit_2bytes" || F.getName() == "emit_byte") {
    if (MF.begin() != MF.end()) {
