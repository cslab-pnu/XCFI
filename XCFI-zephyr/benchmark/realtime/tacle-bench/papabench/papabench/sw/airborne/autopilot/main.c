...
#define DEADLINE_20Hz 50000ULL
#define DEADLINE_10Hz 100000ULL
#define DEADLINE_4Hz 2500000ULL
...
/* scheduler stats */
static uint64_t sched_period_sum = 0;
static uint64_t sched_period_max = 0;
static uint32_t sched_period_count = 0;

/* task runtime stats */
static uint64_t exec20_sum = 0, exec20_max = 0;
static uint64_t exec10_sum = 0, exec10_max = 0;
static uint64_t exec4_sum  = 0, exec4_max  = 0;

static uint32_t exec20_count = 0;
static uint32_t exec10_count = 0;
static uint32_t exec4_count  = 0;
 ...
 */
static inline void ground_calibrate( void ) {
 ...
 *  \brief update ir estimation if RADIO_LLS is 1 \n
 */
...
void periodic_task( void ) {
  ...
  if (last_cycle) {
	  uint32_t diff = now_cycle - last_cycle;
	  uint64_t period_us =
		  (uint64_t)diff * 1000000ULL / sys_clock_hw_cycles_per_sec();

	sched_period_sum += period_us;

    if (period_us > sched_period_max)
        sched_period_max = period_us;
...
if (_4Hz == 0)
{
  ...
  // printf("time cycles: %u\n", cycles);
	...
	int64_t exec_us =
		(int64_t)cycles * 1000000 / sys_clock_hw_cycles_per_sec();

	exec4_sum += exec_us;

	if (exec_us > exec4_max)
		exec4_max = exec_us;
...
else if (_20Hz == 1)
{
	...
	uint64_t exec_us =
		(uint64_t)cycles * 1000000ULL / sys_clock_hw_cycles_per_sec();

	exec10_sum += exec_us;

	if (exec_us > exec10_max)
		exec10_max = exec_us;
...
{
    printf("\n===== PapaBench Timing Results =====\n");

    printf("Scheduler period avg : %llu us\n",
           sched_period_sum / sched_period_count);

    printf("Scheduler period max : %llu us\n",
           sched_period_max);

    printf("\n20Hz task:\n");
    printf("  WCET : %llu us\n", exec20_max);
    printf("  AVG  : %llu us\n", exec20_sum / exec20_count);
    printf("  DEADLINE  : %llu us\n", DEADLINE_20Hz);

    printf("\n10Hz task:\n");
    printf("  WCET : %llu us\n", exec10_max);
    printf("  AVG  : %llu us\n", exec10_sum / exec10_count);
    printf("  DEADLINE  : %llu us\n", DEADLINE_10Hz);

    printf("\n4Hz task:\n");
    printf("  WCET : %llu us\n", exec4_max);
    printf("  AVG  : %llu us\n", exec4_sum / exec4_count);
    printf("  DEADLINE  : %llu us\n", DEADLINE_4Hz);
 ...
 * launch to 1 (which is not set in non auto launch. Then call
...
void send_takeOff(void){
  if (gps_pos_available){
     if (!estimator_flight_time && (estimator_hspeed_mod > MIN_SPEED_FOR_TAKEOFF)) {
        ...
        launch = 1; /* Not set in non auto launch */
