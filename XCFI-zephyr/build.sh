...
# --- configure board files for a build WITHOUT TF-M ---
...
sed -i -E 's|^([[:space:]]*zephyr,sram = ).*|\1\&isram;|'  "$DTS"
sed -i -E 's|^([[:space:]]*zephyr,flash = ).*|\1\&sram;|'  "$DTS"
sed -i -E 's|^(CONFIG_ROMSTART_RELOCATION_ROM=).*|\1n|'    "$DEFCONFIG"

COMPILE_MODE="${2:-xcfi}"
CFLGAS=""

if [ "$COMPILE_MODE" = "base" ]; then
    CFLAGS="$CFLAGS -fno-jump-tables"
    cp -r ../arm-compiler-rt/base/lib ../llvm-install/lib/clang/21/
    cp -r ../arm-compiler-rt/base/include ../llvm-install/lib/clang/21/
    else #xcfi
        CFLAGS="$CFLAGS -mbranch-protection=bti+pac-ret -mllvm -enable-XCFI=all -DENABLE_PACBTI -DENABLE_XCFI"
    	cp -r ../arm-compiler-rt/xcfi/lib ../llvm-install/lib/clang/21/
    	cp -r ../arm-compiler-rt/xcfi/include ../llvm-install/lib/clang/21/
fi

# trio-snprintf
#CFLAGS="$CFLAGS -DTRIO_SNPRINTF -DTRIO_SNPRINTF_ONLY -DTRIO_EXTENSION=0 -DTRIO_DEPRECATED=0 -DTRIO_MICROSOFT=0 -DTRIO_ERRORS=0 -DTRIO_FEATURE_FLOAT=0"

# trio-sscanf
#CFLAGS="$CFLAGS -DTRIO_SSCANF -DTRIO_FEATURE_FLOAT=0 -DTRIO_FEATURE_FILE=0 -DTRIO_FEATURE_STDIO=0 -DTRIO_FEATURE_FD=0 -DTRIO_FEATURE_DYNAMICSTRING=0 \
...
west build -p always -b mps3/corstone310/an555 $1 -DZEPHYR_TOOLCHAIN_VARIANT=llvm -DCMAKE_C_FLAGS="$CFLAGS" \
    -DCMAKE_ASM_FLAGS="$CFLAGS" -DCMAKE_CXX_FLAGS="$CFLAGS"
